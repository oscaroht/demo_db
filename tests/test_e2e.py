...
@pytest.fixture
def mock_buffer_manager(mock_table_data): # Renamed fixture
    """Creates the MockBufferManager object from the raw data map."""
    return MockBufferManager(mock_table_data)

@pytest.fixture
def planner(mock_catalog, mock_buffer_manager): # Planner now accepts the mock BM
...
def test_select_all_with_filter(planner):
...
def test_select_projection_with_filter(planner):
...
def test_select_distinct_single_column(planner):
...
def test_select_distinct_multiple_columns(planner):
...
def test_select_distinct_with_filter(planner):
...
def test_global_aggregate_count_star(planner):
...
def test_group_by_with_count_and_avg(planner):
...
def test_distinct_with_order_by(planner):
...
def test_select_order_by_desc(planner):
...
def test_select_order_by_asc(planner):
    """Test SELECT with ORDER BY (requires Pre-Projection)."""
...
def test_distinct_with_order_by_limit(planner):
    """Test SELECT DISTINCT combined with ORDER BY (requires Pre-Projection)."""
...
def test_aggregate_with_filter_and_order_by(planner):
...
def test_select_distinct_on_all_columns_returns_full_dataset(planner):
    """
...
def test_order_by_multi_key_asc(planner):
    """
